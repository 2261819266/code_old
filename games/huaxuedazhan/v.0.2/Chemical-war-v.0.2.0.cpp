...
This version Creation date:2021.8.4 8:36:26.(v.0.2.0)
Completion data:2021.8.5 16:25:46
file size: 15.8KB
...
bool make(int k, int x) {
    ...
    if (nn > 1) {
        ...
        for (int c = 1; c <= make_thing_n[x] && g < nn; c++) {
            int e;
            if (Is_thing_make_false(k, e = make_thing_way[x][c])) {
                printf("\t%d=>%s\n", ++g, f[e].contest);
            }
        }
        printf(_STR11);
